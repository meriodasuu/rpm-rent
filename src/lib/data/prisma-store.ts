      ...
      const result = await getPrisma().$transaction(async (database) => {
        ...
        return database.booking.create({
          data: {
            ...
            birthDate: toDatabaseDate(input.birthDate),
            licenseIssuedOn: toDatabaseDate(input.licenseIssuedOn),
            driverAgeAtStart: prepared.eligibility.ageAtStart,
            drivingExperienceMonths: prepared.eligibility.experienceMonths,
            minimumAgeApplied: prepared.eligibility.effectiveMinimumAge,
            minimumExperienceApplied: selectedCar.minimumDrivingExperience,
