    ...
    <div className="page contacts-page"><div className="container">
      ...
      <div className="contacts-layout">
        <section className="surface contacts-card">
          ...
          <div className="social-links" aria-label="Социальные сети RPM Rent">{CONTACTS.socials.map((social, index) => { const Icon = socialIcons[index] ?? Globe2; const event = social.label === "Telegram" ? "telegram_click" : social.label === "MAX" ? "max_click" : undefined; return <a aria-label={social.label} href={social.href} key={social.label} rel="noreferrer" target="_blank" title={social.label} data-event={event} data-event-label="contacts"><Icon aria-hidden size={22} /><span>{social.label}</span></a>; })}</div>
