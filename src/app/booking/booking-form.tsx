...
export function BookingForm({ cars, services, initialCarSlug, initialStart, initialEnd, initialServiceSlug }: { cars: Car[]; services: Service[]; initialCarSlug?: string; initialStart?: string; initialEnd?: string; initialServiceSlug?: string }) {
  ...
  const eligibility = useMemo(() => {
    ...
    try {
      const result = evaluateDriverEligibility({ birthDate, licenseIssuedOn, startDate: startAt, minimumAge: selectedCar.minimumAge!, minimumExperienceMonths: selectedCar.minimumDrivingExperience! });
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!canSubmit) { setError(eligibility.message || policyProblem || "Проверьте срок аренды"); return; }
    setPending(true);
    setError("");
    const data = new FormData(form);
    const params = new URLSearchParams(window.location.search);
    const utm = Object.fromEntries([...params.entries()].filter(([key]) => key.startsWith("utm_")).slice(0, 20));
    const payload = { carId, startAt, endAt, pickupMethod, deliveryAddress: String(data.get("deliveryAddress") ?? ""), customerName: String(data.get("customerName") ?? ""), phone: String(data.get("phone") ?? ""), telegram: String(data.get("telegram") ?? ""), birthDate, licenseIssuedOn, additionalServiceIds: selectedServices, comment: String(data.get("comment") ?? ""), privacyConsent: data.get("privacyConsent") === "on", utm, referrer: document.referrer, idempotencyKey: idempotencyKey.current };
    try {
  ...
  return <form className="booking-layout" onSubmit={submit} noValidate data-event="booking_submit" data-event-label={selectedCar.slug}>
    <div className="surface booking-form">
      ...
      <section className="form-section"><h2>3. Данные водителя</h2><p className="form-hint">Клиент является водителем. Возраст и стаж рассчитываются сервером на дату начала аренды.</p><div className="form-grid"><div className="field"><label htmlFor="customerName">Имя</label><input className="input" id="customerName" name="customerName" autoComplete="name" required minLength={2} maxLength={100} /></div><div className="field"><label htmlFor="phone">Телефон</label><input className="input" id="phone" name="phone" type="tel" autoComplete="tel" placeholder="+7 999 123-45-67" inputMode="tel" pattern="[+0-9() \-]{7,30}" minLength={7} maxLength={30} title="Введите от 7 до 15 цифр; допустимы +, пробелы, скобки и дефисы" required /></div><div className="field"><label htmlFor="telegram">Telegram</label><input className="input" id="telegram" name="telegram" placeholder="@username" pattern="@?[A-Za-z0-9_]{5,32}" maxLength={33} /></div><div className="field"><label htmlFor="birthDate">Дата рождения</label><input className="input" id="birthDate" name="birthDate" type="date" value={birthDate} max={today} onChange={(event) => setBirthDate(event.target.value)} required /></div><div className="field full"><label htmlFor="licenseIssuedOn">Дата выдачи водительского удостоверения</label><input className="input" id="licenseIssuedOn" name="licenseIssuedOn" type="date" value={licenseIssuedOn} min={birthDate || undefined} max={startAt} onChange={(event) => setLicenseIssuedOn(event.target.value)} required /></div></div><p className={eligibility.ok ? "availability-note" : "form-hint"}>{eligibility.message}</p></section>
    ...
    <aside className="surface summary"><div className="summary-image"><Image src={selectedCar.images[0]?.url ?? "/images/cars/porsche-911-carrera-4s/01.jpg"} alt={selectedCar.images[0]?.alt ?? selectedCar.title} fill preload sizes="380px" /></div><div className="summary-content"><span className="car-kicker">Сводка оформления</span><h2 className="car-title">{selectedCar.title}</h2><div className="summary-lines"><div className="summary-line"><span>Срок</span><strong>{calculation.days || "Не выбран"} сут.</strong></div><div className="summary-line"><span>Ставка</span><strong>{formatPrice(selectedCar.pricePerDay)} / сут.</strong></div><div className="summary-line"><span>Аренда</span><strong>{formatPrice(calculation.rentalPrice)}</strong></div><div className="summary-line"><span>Услуги</span><strong>{formatPrice(calculation.servicesPrice)}</strong></div><div className="summary-line"><span>Залог отдельно</span><strong>{formatPrice(calculation.deposit)}</strong></div></div><div className="summary-total"><span>Предварительно</span><span>{formatPrice(calculation.dueWithoutDeposit)}</span></div><button className="button red" style={{ width: "100%", marginTop: 22 }} type="submit" disabled={pending || !canSubmit}>{pending ? <><LoaderCircle size={17} className="spin" /> Отправляем…</> : policyProblem ? "Онлайн-оформление недоступно" : "Отправить обращение"}</button><p className="muted small" style={{ display: "flex", gap: 8, marginTop: 16 }}><ShieldCheck size={16} /> Сервер повторно проверит даты, требования водителя, цену и отсутствие пересечений.</p></div></aside>
  </form>;
}
