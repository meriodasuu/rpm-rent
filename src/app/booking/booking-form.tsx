...
import { bookingPolicyProblem, evaluateDriverEligibility, hasConfiguredBookingPolicy } from "@/lib/domain/booking";
...
export function BookingForm({ cars, services, initialCarSlug, initialStart, initialEnd, initialServiceSlug }: { cars: Car[]; services: Service[]; initialCarSlug?: string; initialStart?: string; initialEnd?: string; initialServiceSlug?: string }) {
  ...
  const [licenseIssuedOn, setLicenseIssuedOn] = useState("");
  ...
  const calculation = useMemo(() => {
    ...
    try {
      return calculateRental({
        ...
        servicePrices: services.filter((item) => selectedServices.includes(item.id)).map((item) => item.price ?? 0)
      });
    } catch {
  ...
  const eligibility = useMemo(() => {
    if (!selectedCar || policyProblem) return { ok: false, message: policyProblem ?? "" };
    if (!birthDate || !licenseIssuedOn) return { ok: false, message: "Укажите дату рождения и дату выдачи водительского удостоверения. Стаж посчитается автоматически." };
    try {
      const result = evaluateDriverEligibility({
        birthDate,
        licenseIssuedOn,
        startDate: startAt,
        minimumAge: selectedCar.minimumAge!,
        minimumExperienceMonths: selectedCar.minimumDrivingExperience!
      });
      return { ok: true, message: `На начало аренды: ${result.ageAtStart} лет, подтверждённый стаж ${result.experienceMonths} мес.` };
    } catch (caught) {
      return { ok: false, message: caught instanceof Error ? caught.message : "Проверьте данные водителя" };
    }
  }, [birthDate, licenseIssuedOn, policyProblem, selectedCar, startAt]);

  const minimumDaysMet = Boolean(selectedCar?.minimumRentalDays && calculation.days >= selectedCar.minimumRentalDays);
  const canSubmit = !policyProblem && eligibility.ok && minimumDaysMet && calculation.days <= RENTAL_POLICY.maximumRentalDays;
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!canSubmit) {
      setError(eligibility.message || policyProblem || "Проверьте срок аренды");
      return;
    }
    setPending(true);
    setError("");
  ...
  if (success) return <div className="surface success-panel"><div className="success-icon"><Check size={28} /></div><h1>Заявка отправлена</h1><p className="subtitle" style={{ marginInline: "auto" }}>Менеджер подтвердит доступность автомобиля и свяжется с вами. Заявка не является автоматическим подтверждением брони.</p><div className="button-row" style={{ justifyContent: "center", marginTop: 26 }}><Link className="button" href="/cars">Вернуться в каталог</Link><Link className="button ghost" href="/contacts">Контакты</Link></div></div>;

  return (
    <form className="booking-layout" onSubmit={submit} noValidate data-event="booking_submit" data-event-label={selectedCar.slug}>
      <div className="surface booking-form">
        <section className="form-section">
          <h2>1. Автомобиль и даты</h2>
          <p className="form-hint">Срок считается по календарным суткам: дата возврата не входит в оплачиваемый период.</p>
          ...
          {policyProblem ? <p className="field-error" role="status"><AlertTriangle size={16} /> {policyProblem}. Онлайн-заявка отключена до публикации точных требований; <Link href="/contacts">свяжитесь с менеджером</Link>.</p> : <p className="form-hint">Минимальный срок: {selectedCar.minimumRentalDays} сут.; возраст от {Math.max(RENTAL_POLICY.legalAdultAge, selectedCar.minimumAge ?? 0)} лет; стаж от {selectedCar.minimumDrivingExperience} мес.</p>}
        </section>
        ...
        <section className="form-section">
          <h2>3. Данные водителя</h2>
          <p className="form-hint">Заявитель является водителем. Возраст и стаж рассчитываются сервером на дату начала аренды.</p>
          <div className="form-grid">
            <div className="field"><label htmlFor="customerName">Имя</label><input className="input" id="customerName" name="customerName" autoComplete="name" required minLength={2} maxLength={100} /></div>
            <div className="field"><label htmlFor="phone">Телефон</label><input className="input" id="phone" name="phone" type="tel" autoComplete="tel" placeholder="+7 999 123-45-67" inputMode="tel" pattern="[+0-9() \-]{7,30}" minLength={7} maxLength={30} title="Введите от 7 до 15 цифр; допустимы +, пробелы, скобки и дефисы" required /></div>
            <div className="field"><label htmlFor="telegram">Telegram</label><input className="input" id="telegram" name="telegram" placeholder="@username" pattern="@?[A-Za-z0-9_]{5,32}" maxLength={33} /></div>
            <div className="field"><label htmlFor="birthDate">Дата рождения</label><input className="input" id="birthDate" name="birthDate" type="date" value={birthDate} max={today} onChange={(event) => setBirthDate(event.target.value)} required /></div>
            <div className="field full"><label htmlFor="licenseIssuedOn">Дата выдачи водительского удостоверения</label><input className="input" id="licenseIssuedOn" name="licenseIssuedOn" type="date" value={licenseIssuedOn} min={birthDate || undefined} max={startAt} onChange={(event) => setLicenseIssuedOn(event.target.value)} required /></div>
          </div>
          <p className={eligibility.ok ? "availability-note" : "form-hint"}>{eligibility.message}</p>
        </section>

        <section className="form-section"><h2>4. Дополнительные услуги</h2><div className="grid-2">{services.filter((item) => item.slug !== "daily" && item.slug !== "long-term").map((service) => <label className="condition" key={service.id}><input type="checkbox" checked={selectedServices.includes(service.id)} onChange={() => setSelectedServices((current) => current.includes(service.id) ? current.filter((item) => item !== service.id) : [...current, service.id])} /><span><strong>{service.title}</strong><span className="muted small" style={{ display: "block", marginTop: 4 }}>{service.price ? formatPrice(service.price) : "Без доплаты"}</span></span></label>)}</div><div className="field" style={{ marginTop: 20 }}><label htmlFor="comment">Комментарий</label><textarea className="textarea" id="comment" name="comment" maxLength={1000} placeholder="Пожелания, удобное время для связи и другие детали" /></div></section>

        <section className="form-section"><label className="condition"><input type="checkbox" name="privacyConsent" required /><span><strong>Согласие на обработку данных</strong><span className="muted small" style={{ display: "block", marginTop: 4 }}>Я ознакомился с <Link href="/privacy" target="_blank">политикой конфиденциальности</Link> и согласен передать данные для обработки заявки.</span></span></label></section>
        {error ? <p className="field-error" role="alert" style={{ marginTop: 20 }}>{error}</p> : null}
      </div>

      <aside className="surface summary">
        <div className="summary-image"><Image src={selectedCar.images[0]?.url ?? "/images/cars/porsche-911-carrera-4s/01.jpg"} alt={selectedCar.images[0]?.alt ?? selectedCar.title} fill preload sizes="380px" /></div>
        <div className="summary-content"><span className="car-kicker">Сводка заявки</span><h2 className="car-title">{selectedCar.title}</h2><div className="summary-lines"><div className="summary-line"><span>Срок</span><strong>{calculation.days || "Не выбран"} сут.</strong></div><div className="summary-line"><span>Ставка</span><strong>{formatPrice(selectedCar.pricePerDay)} / сут.</strong></div><div className="summary-line"><span>Аренда</span><strong>{formatPrice(calculation.rentalPrice)}</strong></div><div className="summary-line"><span>Услуги</span><strong>{formatPrice(calculation.servicesPrice)}</strong></div><div className="summary-line"><span>Залог отдельно</span><strong>{formatPrice(calculation.deposit)}</strong></div></div><div className="summary-total"><span>Предварительно</span><span>{formatPrice(calculation.dueWithoutDeposit)}</span></div><button className="button red" style={{ width: "100%", marginTop: 22 }} type="submit" disabled={pending || !canSubmit}>{pending ? <><LoaderCircle size={17} className="spin" /> Отправляем…</> : policyProblem ? "Онлайн-заявка недоступна" : "Отправить заявку"}</button><p className="muted small" style={{ display: "flex", gap: 8, marginTop: 16 }}><ShieldCheck size={16} /> Сервер повторно проверит даты, требования водителя, цену и отсутствие пересечений.</p></div>
      </aside>
    </form>
  );
}
