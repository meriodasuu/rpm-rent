...
export default async function ServicesPage() {
  ...
  return (
    <div className="page services-page"><div className="container">
      ...
      <div className="service-detail-grid">{services.map((service, index) => { const Icon = icons[index % icons.length] ?? CarFront; const context = getServiceContext(service); return <article className="service-detail-card" key={service.id}><div className="service-detail-top"><div className="icon-box"><Icon size={20} /></div><span>{String(index + 1).padStart(2, "0")}</span></div><h2>{service.title}</h2><p>{service.description}</p><div className="service-context"><strong>Кому подойдёт</strong><span>{context.audience}</span></div><div className="service-context"><strong>Как заказать</strong><span>{context.order}</span></div><div className="service-detail-bottom"><span>{service.price ? `от ${formatPrice(service.price)}` : "Стоимость по согласованию"}</span><Link href={`/booking?service=${service.slug}`} data-event="service_open" data-event-label={service.slug}>Добавить в заявку <ArrowRight size={15} /></Link></div></article>; })}</div>

      <section className="section"><div className="section-head section-head-copy"><div><h2 className="title">Сначала маршрут, затем опции</h2></div><p className="section-lead">Заявка остаётся короткой, а менеджер получает достаточно контекста для точного ответа.</p></div><div className="steps"><article className="step"><h3>Определите период</h3><p>Один день, выходные или продолжительный срок.</p></article><article className="step"><h3>Выберите автомобиль</h3><p>Сравните кузов, класс, ставку и залог.</p></article><article className="step"><h3>Добавьте услугу</h3><p>Отметьте нужный вариант или опишите задачу.</p></article><article className="step"><h3>Получите подтверждение</h3><p>Менеджер проверит автомобиль, период и возможность услуги.</p></article></div></section>
      ...
      <section className="section"><div className="cta-panel"><div className="cta-content"><h2 className="title">Выберите автомобиль для заявки</h2><div className="button-row"><Link className="button red" href="/cars">Открыть каталог <ArrowRight size={18} /></Link></div></div></div></section>
    </div></div>
  );
}
