...
export function CarCard({ car, eager = false, period, periodAvailable }: { car: Car; eager?: boolean; period?: { start?: string; end?: string }; periodAvailable?: boolean }) {
  ...
  return (
    <article className="car-card">
      <Link href={detail} aria-label={`Подробнее: ${car.title}`} data-event="car_card_open" data-event-label={car.slug}><div className="car-image">{image ? <Image src={image.url} alt={image.alt} fill sizes="(max-width: 760px) 100vw, (max-width: 1100px) 50vw, 33vw" loading={eager ? "eager" : "lazy"} /> : null}<div className="car-tags">{car.isNew ? <span className="tag">Новинка</span> : null}{car.isPromotion ? <span className="tag red">Спецусловие</span> : null}{!bookable ? <span className="tag">{busyForPeriod ? "Занят на даты" : "Заявка недоступна"}</span> : null}</div><span className="car-image-index">RPM / {car.brand}</span></div></Link>
      <div className="car-content"><span className="car-kicker">{car.category} · {car.bodyType}</span><h3 className="car-title"><Link href={detail} data-event="car_card_open" data-event-label={car.slug}>{car.title}</Link></h3><div className="car-specs">{car.year ? <span>{car.year}</span> : null}{car.transmission ? <span>{car.transmission}</span> : null}{car.driveType ? <span>{car.driveType}</span> : null}<span>{car.vehicleClass}</span></div><div className="car-footer"><div><div className="price">{formatPrice(car.pricePerDay)} <small>/ сутки</small></div><span className="availability-note">{availabilityText}</span></div><Link className="button small-button" href={bookable ? booking : detail} data-event={bookable ? "booking_open" : "car_card_open"} data-event-label={car.slug}>{bookable ? <>Заявка <ArrowRight size={15} /></> : <>Подробнее <ArrowRight size={15} /></>}</Link></div></div>
    </article>
  );
}
