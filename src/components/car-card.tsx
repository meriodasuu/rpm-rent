...
export function CarCard({ car, eager = false, period, periodAvailable }: { car: Car; eager?: boolean; period?: { start?: string; end?: string }; periodAvailable?: boolean }) {
  ...
  return (
    <article className="car-card">
      <Link href={detail} aria-label={`Подробнее: ${car.title}`} data-event="car_card_open" data-event-label={car.slug}><div className="car-image">{image ? <Image src={image.url} alt={image.alt} fill sizes="(max-width: 760px) 100vw, (max-width: 1100px) 50vw, 33vw" loading={eager ? "eager" : "lazy"} /> : null}<div className="car-tags">{car.isNew ? <span className="tag">Новинка</span> : null}{car.isPromotion ? <span className="tag red">Спецусловие</span> : null}{!bookable ? <span className="tag">{busyForPeriod ? "Занят на даты" : "Онлайн-оформление недоступно"}</span> : null}</div><span className="car-image-index">RPM / {car.brand}</span></div></Link>
